...
def moves(tab):     # encontra a posição do espaço sem nenhuma peça
    for i in range(3):
        for j in range(3):
            if tab[i][j] == 0:
                x, y = i, j
    movs = []       # adiciona movimentos possiveis com base na posição do espaço sem nada no tabuleiro
...
def mover(tab, mov):      # cria uma copia do tabuleiro para aplicar o movimento
    novo = [linha[:] for linha in tab]   
    for i in range(3):   # encontra a posição do espaço sem nenhuma peça
        for j in range(3):
            if tab[i][j] == 0:
                x, y = i, j
    if mov == 'C' and x > 0:     # faz movimento solicitado, troca o espaço sem nada com o espaço do lado
...
def mostrar(tab):       # mostra o tabuleiro formatado no terminal
    print("+---+---+---+")
    for linha in tab:
